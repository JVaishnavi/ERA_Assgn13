...
def test():
    ...
    dataset = YOLODataset(
        "PASCAL_VOC/1examples.csv",
        "PASCAL_VOC/image_sample/",
        "PASCAL_VOC/label_sample/",
        S=[13, 26, 52],
        anchors=anchors,
        transform=transform,
    )
    ...
    for x_test, y_test in loader:
        ...
        for i in range(y_test[0].shape[1]):
            ...
            boxes += cells_to_bboxes(
                y_test[i], is_preds=False, S=y_test[i].shape[2], anchors=anchor
            )[0]
        boxes = nms(boxes, iou_threshold=1, threshold=0.7, box_format="midpoint")
        #print(boxes)
        plot_image(x_test[0].permute(1, 2, 0).to("cpu"), boxes)
